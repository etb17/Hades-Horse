...
def load_characterimg(file_path):
    img = pygame.image.load(file_path)
    img = pygame.transform.scale(img, (50, GRID_SIZE))
...
# Images
hero_walk1 = load_characterimg("assets/character/adventurer_walk1.png")
hero_walk2 = load_characterimg("assets/character/adventurer_walk2.png")
hero_jump = load_characterimg("assets/character/adventurer_jump.png")
hero_idle = load_characterimg("assets/character/adventurer_idle.png")
...
health_img = load_image("assets/items/health.png")
empty_health_img = load_image("assets/items/empty_health.png")
lives_img = load_image("assets/items/lives.png")
ladder_img = load_image("assets/items/ladder.png")
...
MONSTER_DEATH = pygame.mixer.Sound("assets/sounds/monster_death.wav")
...
class Character(Entity):
    ...
    def jump(self, blocks, level):
        ...
        if level.is_underwater == 0 and not self.on_ladder:
        ...
        elif level.is_underwater == 1 and not self.on_ladder:
            self.vy = -1 * self.jump_power
            play_sound(JUMP_SOUND)
    ...
    def process_powerups(self, powerups):
        hit_list = pygame.sprite.spritecollide(self, powerups, False)
...
class Level():
    ...
    def __init__(self, file_path):
        ...
        self.active_sprites.add(self.coins, self.enemies, self.powerups, self.ladder)
        self.inactive_sprites.add(self.blocks, self.flag)
        self.clock_extra = map_data['clockextra']
        self.inactive_sprites.draw(self.inactive_layer)
        self.time_remaining = FPS * (self.clock_extra + 60)
        self.is_underwater = map_data['is_underwater']
        #is_underwater should either be 0 or 1
...
class Game():
    ...
    def process_events(self):
        ...
        buttons = [a_btn, b_btn, x_btn, y_btn, back, start]
        ...
        if self.stage == Game.PLAYING:
            if round(left_x) == -1:
                self.hero.move_left(lt_y, b_btn)
            elif round(left_x) == 1:
                self.hero.move_right(lt_y, b_btn)
            else:
                self.hero.stop()
    ...
    def update(self):
        ...
        if self.level.time_remaining <= 0:
            self.level.time_remaining = 0
            self.hero.hearts -= 1
